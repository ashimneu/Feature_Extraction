function [features] = compute_feature_using_sw(p,PointCloud,sz_window)
    ...
    [L,P,S,O,A,Esum,SV,Entropy,Sm,FOFA,FOSA,SOFA,SOSA] = deal(nan(num_pnts,1));
    features = cell(1,13); % number of features to extract
        ...
        if ~isempty(pc_sw)
            ...
            if (dis_sep < valid_range) % only analyze when all points are within a valid small range of slide window
                ...
                case 1
                    ...
                    %[medoid,~] = mfames(pc_sw'); % true medoid (computationally expensive)                
                    % using mid point in sliding window as close approximation 
                    % of medoid since medoid calculation is computationally expensive.
                    medoid = pc_sw(pnt_ref_idx,:);
                case 2
                    pnt_ref = pc(i,:); % reference point USE ME if using SWindowknnSearch_v1/v2
                    pnt_ref_idx = hf_sz+1; % index of the central point in the sliding window; USE ME if using SWindowknnSearch_v1/v2
                    %[medoid,~] = mfames(pc_sw'); % true medoid (computationally expensive)
                    medoid = pc_sw(pnt_ref_idx,:); % mid point of sliding window. See case 1 for more details.
                case 3
                    pnt_ref_idx = pnt_ref_i;
                    pnt_ref = pc_sw(pnt_ref_idx,:); % reference point USE ME if using SWindowInRadiusSearch
                    [medoid,~] = mfames(pc_sw'); % true medoid (computationally expensive)
                ...
                [L(i),P(i),S(i),O(i),A(i),Esum(i),SV(i),Entropy(i),FOFA(i),FOSA(i),SOFA(i),SOSA(i),Sm(i)] = compute_features(pc_sw,pnt_ref_idx);
%                 pnt_ref = pc_sw(pnt_ref_idx,:);
%                 [FOFA(i),FOSA(i),SOFA(i),SOSA(i)] = moments(pc_sw,pnt_ref,U);
            end
        end
    end
    [features{:}] = deal(L,P,S,O,A,Esum,SV,Entropy,Sm,FOFA,FOSA,SOFA,SOSA);
end

function [L,P,S,O,A,Esum,SV,Entropy,FOFA,FOSA,SOFA,SOSA,Sm] = compute_features(pc_sw,pnt_ref_idx)
    ...
    % U  - matrix of orthogonal eigen vectors corresponding to entries in SV
