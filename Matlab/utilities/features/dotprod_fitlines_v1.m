function [dotprod,theta,len_pr,len_sc] = dotprod_fitlines(p,pc,pnt_ref_idx)
    ...
    if idx_is_invalid
        ...
        pr_pnts = translated_pc(1:pnt_ref_idx,:); % preceeding points
        sc_pnts = translated_pc(pnt_ref_idx:end,:); % succeeding points
        ...
        dotprod = abs(dot(vec_pr,vec_sc)); % dot product
        ...
        %     
        %         % plot eigen vectors
        %         plot3([0 u1(1)],[0 u1(2)],[0 u1(3)],'-k')
        %         plot3([0 u2(1)],[0 u2(2)],[0 u2(3)],'-m')
